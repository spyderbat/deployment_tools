...
def get_nodegroup_info(region, cluster_name = 'all'):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--eks_cluster_name', default = 'all', help='EKS cluster name (default is all clusters)')
    parser.add_argument('-o', '--output', default='stdout', help='Output file')
    args = parser.parse_args()
    info = get_nodegroup_info(args.region, args.eks_cluster_name)
    output = sys.stdout if args.output == 'stdout' else open(args.output, 'w')
    json.dump(info, output, default=str)
